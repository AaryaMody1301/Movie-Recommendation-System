...
DEVELOPMENT_SECRET_KEY = "dev-secret-key-change-in-production"
INSECURE_SECRET_KEYS = frozenset({DEVELOPMENT_SECRET_KEY, "change-me"})
...
class DevelopmentConfig(Config):
    ...
    DEBUG = True
    SECRET_KEY = os.environ.get("SECRET_KEY") or DEVELOPMENT_SECRET_KEY
...
def get_config():
    ...
    try:
        ...
        allowed = ", ".join(sorted(config))
        raise RuntimeError(f"Unsupported FLASK_ENV={env!r}; expected one of: {allowed}") from exc
